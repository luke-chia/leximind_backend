...
export class RAGAdapter {
  ...
  async processQuery(
    ...
    const { areas, topK = 10, filters, systemPrompt } = options

    const executionId = `[${new Date().toLocaleTimeString()}.${Date.now() % 1000}]`

    try {
      console.log(
        `RagAdapter.processQuery --> ${executionId} 🤔 Procesando pregunta RAG: "${question}"`
      )
      console.log(
        `RagAdapter.processQuery --> ${executionId} 🔍 Buscando ${topK} documentos relevantes...`
      )
      ...
      if (!searchResult.documents || searchResult.documents.length === 0) {
        console.log(
          `RagAdapter.processQuery --> ${executionId} ❌ No se encontraron documentos relevantes`
        )
        return {
          question,
          answer:
            'No se encontraron documentos relevantes para responder tu pregunta. Por favor, intenta reformular tu consulta o verifica que existan documentos relacionados con el tema.',
      ...
      console.log(
        `RagAdapter.processQuery --> ${executionId} ✨ Encontrados ${searchResult.documents.length} documentos relevantes`
      )
      console.log(
        `RagAdapter.processQuery --> ${executionId} 📋 Fuentes encontradas:`
      )

      searchResult.documents.slice(0, 10).forEach((doc, i) => {
        const pageInfo = doc.page !== 'N/A' ? ` | Pág. ${doc.page}` : ''
        console.log(
          `RagAdapter.processQuery --> ${executionId}   ${i + 1}. ${doc.source}${pageInfo} (similitud: ${doc.score.toFixed(3)})`
        )
        console.log(
          `RagAdapter.processQuery --> ${executionId}      📝 ${doc.text.substring(0, 150)}...`
        )
      })
      ...
      const context = searchResult.documents
        .map(
          (doc, i) =>
            ` Documento/Source ${doc.source} (Página/Page ${doc.page}): ${doc.text}`
        )
        .join('\n\n')

      // 4. Generar respuesta usando el LLM con el contexto
      console.log(
        `RagAdapter.processQuery --> ${executionId} 🧠 Generando respuesta con IA...`
      )
      console.log(
        `RagAdapter.processQuery --> ${executionId} Contexto Usado: ${context}`
      )
  ...
  async healthCheck(): Promise<{
    ...
    try {
      ...
      console.log(
        `✅ Estado de servicios - OpenAI: ${openaiStatus ? '✅' : '❌'}, Pinecone: ${pineconeStatus ? '✅' : '❌'}`
      )
