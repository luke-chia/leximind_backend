...
describe('ProcessMessageUseCaseImpl', () => {
    ...
    describe('execute', () => {
        it('should process message and return response DTO', async () => {
            ...
            const mockSource = new Source('5', 'Artículo 5: Todo cliente debe someterse a un proceso de KYC...', 'Manual_PLD.pdf', 'doc-123', '0.95', 'https://storage.supabase.co/documents/doc-123.pdf');
            const mockLLMResponse = new LLMResponse('Los requisitos de KYC incluyen verificación de identidad...', new Date(), [mockSource]);
        ...
        it('should handle message without metadata filters', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Respuesta simple', new Date(), []);
            vi.mocked(mockLLMRepository.processMessage).mockResolvedValue(mockLLMResponse);
            // Act
            const result = await useCase.execute(inputDto);
            // Assert
        ...
        it('should propagate repository errors', async () => {
            ...
            vi.mocked(mockLLMRepository.processMessage).mockRejectedValue(new Error(errorMessage));
            // Act & Assert
        ...
        it('should create ChatMessage with correct timestamp', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Test answer', new Date(), []);
            vi.mocked(mockLLMRepository.processMessage).mockResolvedValue(mockLLMResponse);
            // Act
        ...
        it('should handle multiple sources in response', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Respuesta con múltiples fuentes...', new Date(), mockSources);
            vi.mocked(mockLLMRepository.processMessage).mockResolvedValue(mockLLMResponse);
            // Act
            const result = await useCase.execute(inputDto);
            // Assert
    ...
    describe('Integration with ChatMessage entity', () => {
        it('should validate that ChatMessage is created with all required fields', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Test', new Date(), []);
            vi.mocked(mockLLMRepository.processMessage).mockResolvedValue(mockLLMResponse);
            // Act
            await useCase.execute(inputDto);
            // Assert
