export class ProcessMessageDto {
  ...
  static create(object: { [key: string]: any }): [string?, ProcessMessageDto?] {
    ...
    if (area !== undefined && (!Array.isArray(area) || !area.every(item => typeof item === 'string'))) {
      return ['El campo area debe ser un array de strings']
    }
    if (category !== undefined && (!Array.isArray(category) || !category.every(item => typeof item === 'string'))) {
      return ['El campo category debe ser un array de strings']
    }
    if (source !== undefined && (!Array.isArray(source) || !source.every(item => typeof item === 'string'))) {
    ...
    return [undefined, new ProcessMessageDto(
      userId, 
      message.trim(), 
      area, 
      category, 
