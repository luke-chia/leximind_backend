...
export async function runOnStartup(): Promise<void> {
  try {
    ...
    console.log(`📊 Cache stats: ${stats.totalDocuments} docs, ${(stats.totalSize / 1024 / 1024).toFixed(2)} MB total`)
    
  } catch (err) {
    console.error('❌ Startup: failed to load documents cache from Supabase', err);
