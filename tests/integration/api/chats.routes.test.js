...
describe('Chats API Integration Tests', () => {
    ...
    describe('POST /api/v1/chats/process-message', () => {
        it('should return 200 with valid request', async () => {
            ...
            const mockSource = new Source('5', 'Artículo 5: KYC requirements...', 'Manual_PLD.pdf', 'doc-123', '0.95', 'https://storage.supabase.co/doc-123.pdf');
            const mockLLMResponse = new LLMResponse('Los requisitos de KYC incluyen...', new Date(), [mockSource]);
            // Mock del use case
        ...
        it('should handle empty sources array', async () => {
            ...
            const mockLLMResponse = new LLMResponse('No se encontró información relevante.', new Date(), []);
            vi.mocked(mockProcessMessageUseCase.execute).mockResolvedValue({
                response: mockLLMResponse.response,
                timestamp: mockLLMResponse.timestamp.toISOString(),
                sources: [],
        ...
        it('should handle metadata filters (area, category, tags)', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Respuesta filtrada', new Date(), []);
            vi.mocked(mockProcessMessageUseCase.execute).mockResolvedValue({
                response: mockLLMResponse.response,
                timestamp: mockLLMResponse.timestamp.toISOString(),
                sources: [],
        ...
        it('should return 500 on internal server error', async () => {
            ...
            vi.mocked(mockProcessMessageUseCase.execute).mockRejectedValue(new Error('Database connection failed'));
            // Act
            const response = await request(app)
                .post('/api/v1/chats/process-message')
                .send(requestBody)
        ...
        it('should accept request with only required fields', async () => {
            ...
            const mockLLMResponse = new LLMResponse('Simple answer', new Date(), []);
            vi.mocked(mockProcessMessageUseCase.execute).mockResolvedValue({
                response: mockLLMResponse.response,
                timestamp: mockLLMResponse.timestamp.toISOString(),
                sources: [],
    ...
    describe('API Response Format', () => {
        it('should return properly structured response with all required fields', async () => {
            ...
            const mockSource = new Source('10', 'Test matching text', 'TestDoc.pdf', 'doc-999', '0.88', 'https://test.url');
            const mockLLMResponse = new LLMResponse('Complete test response', new Date('2024-01-15T10:00:00Z'), [mockSource]);
            vi.mocked(mockProcessMessageUseCase.execute).mockResolvedValue({
                response: mockLLMResponse.response,
                timestamp: mockLLMResponse.timestamp.toISOString(),
                sources: [
