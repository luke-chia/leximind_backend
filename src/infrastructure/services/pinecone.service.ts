...
import { VectorDatabaseRepository, VectorSearchFilters } from '../../domain/repositories/vector-database.repository.js'
...
export class PineconeService implements VectorDatabaseRepository {
  ...
  async searchSimilarDocuments(
    ...
    try {
      console.log(`🎯 Buscando ${topK} documentos similares en namespace: ${this.namespace}`)
      ...
      if (results.matches && results.matches.length > 0) {
        console.log(`✅ Primer resultado - ID: ${results.matches[0].id}, Score: ${results.matches[0].score}`)
        ...
        const documents = results.matches.map(match => {
          ...
          return Document.create(
            ...
            match.metadata?.filename ? match.metadata.filename.toString() : 'Desconocida',
            page,
            match.score || 0,
        ...
        console.log(`ℹ️ Sin resultados - posibles causas: filtros restrictivos, embedding sin similitud, o namespace vacío`)
      }
   ...
   */
  private buildMetadataFilters(filters?: VectorSearchFilters, areas?: string[]): Record<string, any> {
  ...
  async ping(): Promise<boolean> {
    try {
      ...
      console.log(`✅ Conexión exitosa con Pinecone - Índice: ${this.indexName}`)
      return true
    } catch (error) {
  ...
  async runDiagnostics(): Promise<void> {
    ...
    try {
      ...
      const indexes = await this.client.listIndexes()
      console.log(`   Índices encontrados: ${indexes.indexes?.map(i => i.name).join(', ') || 'Ninguno'}`)
      
      const targetIndex = indexes.indexes?.find(idx => idx.name === this.indexName)
      if (!targetIndex) {
      ...
      if (stats.namespaces) {
        for (const [namespace, nsStats] of Object.entries(stats.namespaces)) {
          console.log(`     • ${namespace}: ${nsStats.recordCount || 0} vectores`)
        }
      }
      ...
      console.log(`   - Query exitosa: ${testQuery.matches?.length || 0} resultados`)
      if (testQuery.matches && testQuery.matches.length > 0) {
        ...
        console.log(`   - Metadata keys: ${match.metadata ? Object.keys(match.metadata).join(', ') : 'Sin metadata'}`)
      }
  ...
  async upsertVectors(vectors: {
    ...
    try {
      ...
      console.log(`📤 Upserting ${vectors.length} vectors to namespace: ${this.namespace}`)
