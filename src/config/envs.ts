...
console.info('🧪 Variables desde Railway:')
console.info('MONGO_URL:', JSON.stringify(process.env.MONGO_URL))
console.info('OPENAI_API_KEY:', JSON.stringify(process.env.OPENAI_API_KEY))
console.info('PINECONE_API_KEY:', JSON.stringify(process.env.PINECONE_API_KEY))
...
function validateEnvVar(name: string, defaultValue?: string) {
  const val = env
    ...
    .asString()
  if (!val) {
    console.warn(`⚠️ La variable ${name} está vacía o no fue definida`)
  }
  return val
}
